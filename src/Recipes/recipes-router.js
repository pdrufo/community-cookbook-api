...
/**route for each unique recipe in the databse */
recipesRouter
  ...
  .delete((req, res, next) => {
    const { id } = req.params;
    recipesService
      .deleteRecipe(req.app.get("db"), id)
      .then((rowsAffected) => {
        if (rowsAffected < 1) {
  ...
  .patch(bodyParser, (req, res, next) => {
    ...
    recipesService
      .updateRecipe(req.app.get("db"), req.params.recipe_id, recipeToUpdate)
